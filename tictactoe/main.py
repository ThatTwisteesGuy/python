...
def victory_check(current_player):
    ...
    for i in range(3):
        if ("O" in grid[i] and " " not in grid[i] and "X" not in grid[i]) or (
                "O" in grid_r[i] and (" " not in grid_r[i]) and "X" not in grid_r[i]) or (
                grid[0][0] == "O" and grid[1][1] == "O" and grid[2][2] == "O") or (
                grid[0][2] == "O" and grid[1][1] == "O" and grid[2][0] == "O"):
            print("Player O wins!")
            restart()

        if ("X" in grid[i] and " " not in grid[i] and "O" not in grid[i]) or (
                "X" in grid_r[i] and " " not in grid_r[i] and "O" not in grid_r[i]) or (
